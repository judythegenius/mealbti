...
function getMenuKeywordsFromMBTI(mbti: MuckBti): string[] {
  if (mbti.health === 'loss') 
    return ['샐러드', '포케', '샤브샤브', '두부', '그릭요거트볼', '닭가슴살'];
  if (mbti.health === 'gain') 
    return ['소고기', '장어', '삼계탕', '스테이크', '닭갈비', '훠궈'];
  if (mbti.health === 'sugar') 
    return ['현미밥', '두부', '나물', '채소', '저당', '한정식'];

  if (mbti.spicy >= 4) 
    return ['마라탕', '짬뽕', '쭈꾸미', '낙지볶음', '불닭', '떡볶이', '에티오피아'];
  if (mbti.spicy <= 2) 
    return ['칼국수', '돈까스', '초밥', '백반', '크림파스타', '우동', '오므라이스'];

  if (mbti.salty >= 4) 
    return ['족발', '간장게장', '보쌈', '감자탕', '된장찌개', '순대국'];
  if (mbti.salty <= 2) 
    return ['샤브샤브', '백숙', '맑은탕', '월남쌈', '쌀국수', '연어'];

  if (mbti.fullness >= 4 && mbti.drink >= 4) 
    return ['삼겹살', '곱창', '양꼬치', '막창', '소갈비', '해물탕'];
  if (mbti.fullness >= 4) 
    return ['국밥', '감자탕', '한정식', '뷔페', '돼지국밥', '쌈밥'];
  if (mbti.fullness <= 2) 
    return ['김밥', '샌드위치', '브런치', '토스트', '카페밥', '덮밥'];

  if (mbti.drink >= 4) 
    return ['파전', '해물', '보쌈', '치킨', '피자', '이자카야'];
  if (mbti.speed <= 2) 
    return ['오마카세', '코스', '한정식', '브런치카페', '파인다이닝'];
  if (mbti.speed >= 4) 
    return ['분식', '패스트푸드', '편의점', '김밥', '라멘', '덮밥'];

  // fallback - 더 다양하게
  return ['맛집', '파스타', '국밥', '초밥', '버거', '쌀국수', '한식'];
}
  ...
  if (geminiAvailable) {
    try {
      ...
      const contextPayload = {
        user_muck_bti: {
          type: "먹BTI",
          drink_frequency: muckBti.drink >= 4 ? "자주 곁들임" : "거의 안 마심",
          spicy: muckBti.spicy,
          fullness: muckBti.fullness,
          budget: muckBti.budget,
          distance: muckBti.distance,
          speed: muckBti.speed,
          health: muckBti.health
        },
        current_context: {
          meal_type: detectedMealType,
          group_size: groupSize || "1인",
          yesterday_food: yesterdayFood || "없음",
          search_radius_m: radius,
          current_address: req.body.addressText || "서울시"
        },
        is_demo_mode: isDemoMode,
        raw_nearby_restaurants: rawNearby.slice(0, 15).map(r => ({
          name: r.name,
          category: r.category,
          distance_meters: r.distance_meters,
          address: r.address,
          menu_preview: r.menu_preview
        }))
      };
...
if (filteredAndSorted.length < 3) {
  for (const item of sortedCandidates) {
    ...
    curateResults = filteredAndSorted.map(({ rest }) => {
      let defaultMenu = rest.menu_preview[0] || "대체용 특선 요리";
