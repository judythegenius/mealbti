...
function getMenuKeywordsFromMBTI(mbti: MuckBti): string[] {
  if (mbti.health === 'loss') return ['샐러드', '포케', '샤브샤브', '두부'];
  if (mbti.health === 'gain') return ['소고기', '장어', '삼계탕'];
  if (mbti.spicy >= 4) return ['마라탕', '짬뽕', '쭈꾸미볶음', '낙지'];
  if (mbti.spicy <= 2) return ['칼국수', '돈까스', '초밥', '백반'];
  if (mbti.fullness >= 4) return ['국밥', '삼겹살', '감자탕', '한정식'];
  if (mbti.drink >= 4) return ['이자카야', '파전', '곱창', '양꼬치'];
  return ['맛집', '고기', '국수', '덮밥', '찌개'];
}
